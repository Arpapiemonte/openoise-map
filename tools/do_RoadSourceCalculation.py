...
from ui_RoadSourceCalculation import Ui_RoadSourceCalculation_window
...
class Dialog(QDialog,Ui_RoadSourceCalculation_window):
        ...
        for layer in QgsMapLayerRegistry.instance().mapLayers().values():
            try:
                if layer.geometryType() == QGis.Line:
                    roads_layers.append(layer.name())
            except:            
                continue             
    ...
    def update_field_roads_layer(self):
        ...
        roads_layer = QgsMapLayerRegistry.instance().mapLayersByName(self.roads_layer_comboBox.currentText())[0]
        roads_layer_fields = list(roads_layer.dataProvider().fields())
    ...
    def outFile_emission_points(self):
        ...
        if find(self.shapefileName,".shp") == -1 and find(self.shapefileName,".SHP") == -1:
            self.emission_points_layer_lineEdit.setText( self.shapefileName + ".shp")
        else:
    ...
    def controls(self):
        ...
        if self.receiver_points_layer_comboBox.currentText() == "":
            QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr("Please specify the receiver points vector layer."))
            return 0
        
        if self.roads_layer_comboBox.currentText() == "":
            QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr("Please specify the roads line vector layer."))
            return 0
        ...
        if self.vehicles_radioButton.isChecked():
            if self.L_gen_v_checkBox.isChecked():
                if self.surface_comboBox.currentText() == self.tr("choose field") or self.surface_comboBox.currentText() == "========"\
                  ...
                  or (self.L_gen_h_n_comboBox.currentText() == self.tr("choose field") and self.L_gen_h_s_comboBox.currentText() <> self.tr("choose field")):  
                    ...
                    QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr(message))
                    return 0
            ...
            if self.L_gen_v_checkBox.isChecked() == False and self.L_nig_v_checkBox.isChecked() == False\
                ...
                QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr("Please specify at least a reference period."))
                return 0
            if  self.L_den_checkBox.isChecked() and self.L_nig_v_checkBox.isChecked() == False\
                and self.L_day_v_checkBox.isChecked() == False and self.L_eve_v_checkBox.isChecked() == False:
                QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr("Please specify at least a reference period among Lday, Leve or Lnig in order to calculate Lden."))
                return 0
            ...
            if self.L_gen_p_checkBox.isChecked() and self.L_gen_p_comboBox.currentText() == self.tr("choose field"):
                QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr("Please specify the power level field for each reference period to calculate."))
                return 0
        ...
        if  self.L_den_checkBox.isChecked() and int(self.L_day_hours_spinBox.value()) + int(self.L_eve_hours_spinBox.value()) + int(self.L_nig_hours_spinBox.value()) <> 24:
            QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr("The amount of the hours of Lday, Leve and Lnig must be 24."))            
            return 0
        
        if self.obstacles_layer_checkBox.isChecked() and self.obstacles_layer_comboBox.currentText() == "":
            QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr("Please specify the obstacles polygon vector layer."))
            return 0

        if self.emission_points_layer_checkBox.isChecked() and self.emission_points_layer_lineEdit.text() == "":
            QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr("Please specify output shapefile name for the emission points."))          
            return 0
        
        if self.rays_layer_checkBox.isChecked() and self.rays_layer_lineEdit.text() == "":
            QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr("Please specify output shapefile name for the sound rays."))          
            return 0
    ...
    def accept(self):
        ...
        if run == 1:
            ...
            QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr(result_string))
            self.iface.messageBar().pushMessage(self.tr("opeNoise - Road Source Calculation"), self.tr("Process complete"))
        else:
            ...
            QMessageBox.information(self, self.tr("opeNoise - Road Source Calculation"), self.tr(result_string))
            self.iface.messageBar().pushMessage(self.tr("opeNoise - Road Source Calculation"), self.tr("Process not complete"))
    ...
    def log_start(self):
        ...
        log_settings_path_name = os.path.join(dir_path,"log_RoadSourceCalculation_settings.txt")
        log_errors_path_name = os.path.join(dir_path,"log_RoadSourceCalculation_errors.txt")
        log_settings = open(log_settings_path_name,"w")
        log_errors = open(log_errors_path_name,"w")
        log_settings.write(self.tr("opeNoise") + " - " + self.tr("Road Source Calculation") + " - " + self.tr("Settings") + "\n\n")
        log_errors.write(self.tr("opeNoise") + " - " + self.tr("Road Source Calculation") + " - " + self.tr("Errors") + "\n\n")
    ...
    def run(self,receiver_points_layer,roads_layer,roads_layer_details,parameters,obstacles_layer,emission_points_layer_path,rays_layer_path):
        ...
        if emission_points_layer_path <> "":
            ...
            emission_points_layer_name = os.path.splitext(os.path.basename(emission_points_layer_path))[0]
            emission_points_layer = QgsVectorLayer(emission_points_layer_path, unicode(emission_points_layer_name), "ogr")
        ...
        level_fields_new = list(receiver_points_layer.dataProvider().fields())
