...
def get_levels(settings,source_layer,source_feat):
    ...
    # POWER_P    
    if source_layer.geometryType() == QGis.Point and settings['implementation_pts'] == 'True':
        ...
        for key in level_global.keys():
            level_bands[key] = on_Acoustics.GlobalToOctaveBands('pink',level_global[key])
        
    # POWER_R
    elif source_layer.geometryType() == QGis.Line and settings['implementation_roads'] == 'POWER_R':
    ...
    # NMPB
    elif source_layer.geometryType() == QGis.Line and settings['implementation_roads'] == 'NMPB':
    ...
    # CNOSSOS
    elif source_layer.geometryType() == QGis.Line and settings['implementation_roads'] == 'CNOSSOS':
...
#        print 'crea diffraction points ',datetime.now() - time
    ...
    emission_pts_writer = VectorWriter(emission_pts_layer_path, None, emission_pts_fields, 0, receiver_layer.crs())    
...
def run(settings,progress_bars):
    ...
    if diff_rays_layer_path is not None:
        ...
        diff_rays_writer = VectorWriter(diff_rays_layer_path, None, rays_fields, 2, receiver_layer.crs())
    else:
    ...
    if diff_rays_layer_path is not None:
        ...
        diff_rays_layer = QgsVectorLayer(diff_rays_layer_path, unicode(diff_rays_layer_name), "ogr")
    ...
    if len(level_fields_new) > 0:
        ...
        layer = QgsMapLayerRegistry.instance().mapLayersByName(receiver_layer_name)[0]
